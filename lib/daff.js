...
coopy_Coopy.keepAround = function() {
	...
	new coopy_SqlCompare(null,null,null,null);
	new coopy_SqliteHelper();
...
coopy_Coopy.prototype = {
	...
	,encodeTable: function(name,t,render) {
		...
		if(this.format_preference == "xlsx" && name == "-") {
			this.format_preference = "csv";
		}
		if(render == null) {
			if(this.format_preference == "csv") {
				txt = new coopy_Csv(this.delim_preference).renderTable(t);
			} else if(this.format_preference == "xlsx") {
				var bytes = new coopy_Xlsx().renderTable(t);
				if(bytes == null) {
					this.io.writeStderr("! Cannot yet output to xlsx, aborting\n");
					return "";
				}
				this.saveBytes(name,bytes);
				return null;
	...
	,saveTables: function(name,os,use_color) {
		...
		if(this.format_preference == "html" || this.format_preference == "www") {
			...
			var bytes = new coopy_Xlsx().renderTables(os);
			if(bytes == null) {
				this.io.writeStderr("! Cannot yet output to xlsx, aborting\n");
				return false;
			}
			return this.saveBytes(name,bytes);
		}
	...
	,saveBytes: function(name,bytes) {
		if(name != "-") {
			this.io.saveBytes(name,bytes);
		} else {
			this.io.writeStdout(bytes.toString());
		}
		return true;
	}
	,jsonToTables: function(json) {
		if(Reflect.field(json,"tables") == null) {
		...
		while(_g < _g1.length) {
			...
			while(_g3 < _g2) {
				...
				if(!has_hash) {
					...
					while(_g5 < _g4) {
						var j = _g5++;
						output.setCell(j,i,coopy_Coopy.cellFor(lst[j]));
					}
				} else {
					var _g51 = 0;
					...
					while(_g51 < _g41) {
						var j1 = _g51++;
						output.setCell(j1,i,coopy_Coopy.cellFor(Reflect.field(row,columns[j1])));
					}
				}
			}
		}
	...
	,loadTable: function(name) {
		...
		if(ext == "sqlite") {
			...
			return new coopy_SqlTables(sql,this.flags);
		}
		var txt = this.io.getContent(name);
		if(ext == "ndjson") {
			var t = new coopy_SimpleTable(0,0);
			new coopy_Ndjson(t).parse(txt);
			return t;
		}
		if(ext == "json" || ext == "") {
			try {
		...
		if(ext == "xlsx") {
			var bytes = this.io.getBytes(name);
			var output = new coopy_Xlsx().parseTable(bytes);
			if(output != null) {
				return output;
			}
		}
		...
		var output1 = new coopy_SimpleTable(0,0);
		csv.parseTable(txt,output1);
		if(output1 != null) {
			output1.trimBlank();
		}
		return output1;
	...
	,installGitDriver: function(io,formats) {
		...
		if(!(__map_reserved.hello != null?_this.existsReserved("hello"):_this.h.hasOwnProperty("hello"))) {
		...
		if(!(__map_reserved.can_run_git != null?_this2.existsReserved("can_run_git"):_this2.h.hasOwnProperty("can_run_git"))) {
			r = this.command(io,"git",["--version"]);
			if(r == 999) {
				return r;
			}
			var _this3 = this.status;
			if(__map_reserved.can_run_git != null) {
				_this3.setReserved("can_run_git",r);
			} else {
				_this3.h["can_run_git"] = r;
			}
			if(r != 0) {
		...
		if(need_update) {
			io.saveContent(".gitattributes",txt);
		}
		io.writeStdout("- Done!\n");
		return 0;
	}
	,run: function(args,io) {
		...
		if(cmd1 == "git") {
			return this.installGitDriver(io,args.splice(offset,args.length - offset));
		}
		if(git) {
		...
		var a = this.loadTable(args[offset]);
		var b = null;
		...
		if(cmd1 == "diff") {
			...
			new coopy_HighlightPatch(a,b).apply();
			this.saveTable(output,a);
		} else if(cmd1 == "merge") {
			var conflicts = new coopy_Merger(parent,a,b,this.flags).apply();
			ok = conflicts == 0;
			if(conflicts > 0) {
...
var coopy_Csv = $hx_exports["coopy"]["Csv"] = function(delim) {
	if(delim == null) {
		delim = ",";
	}
	this.cursor = 0;
...
var coopy_JsonTable = function(data,name) {
	...
	while(_g1 < _g) {
		var idx = _g1++;
		this.idx2col.h[idx] = this.columns[idx];
	}
	this.name = name;
};
...
coopy_Merger.prototype = {
	...
	,shuffleColumns: function() {
		...
		var fate = [];
		this.local.insertOrDeleteColumns(fate,this.shuffleDimension(this.column_units,this.local.get_width(),fate,this.column_mix_local,this.column_mix_remote));
	}
	,shuffleRows: function() {
		this.row_mix_local = new haxe_ds_IntMap();
		this.row_mix_remote = new haxe_ds_IntMap();
		var fate = [];
		this.local.insertOrDeleteRows(fate,this.shuffleDimension(this.units,this.local.get_height(),fate,this.row_mix_local,this.row_mix_remote));
	}
	,apply: function() {
		this.conflicts = 0;
		this.conflict_infos = [];
		var align = coopy_Coopy.compareTables3(this.parent,this.local,this.remote).align();
		...
		this.flags.allowInsert();
		this.flags.allowDelete();
		this.flags.allowUpdate();
		...
		this.shuffleColumns();
		this.shuffleRows();
		var tmp = this.column_mix_remote.keys();
		while(tmp.hasNext()) {
			var x = tmp.next();
			...
			while(_g4 < _g11.length) {
				...
				if(unit.l >= 0 && unit.r >= 0) {
					this.local.setCell(x2,this.row_mix_local.h[unit.l],this.remote.getCell(x,unit.r));
				} else if(unit.p < 0 && unit.r >= 0) {
					this.local.setCell(x2,this.row_mix_remote.h[unit.r],this.remote.getCell(x,unit.r));
				}
			}
		}
		var tmp1 = this.row_mix_remote.keys();
		while(tmp1.hasNext()) {
			var y = tmp1.next();
			...
			while(_g5 < _g12.length) {
				...
				if(unit1.l >= 0 && unit1.r >= 0) {
					this.local.setCell(this.column_mix_local.h[unit1.l],y2,this.remote.getCell(unit1.r,y));
				}
			}
		}
...
var coopy_SqlTables = $hx_exports["coopy"]["SqlTables"] = function(db,flags) {
	this.db = db;
	var names = this.db.getHelper().getTableNames(db);
	var allowed = null;
	var count = names.length;
...
coopy_SqliteHelper.prototype = {
	getTableNames: function(db) {
		if(!db.begin("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name",null,["name"])) {
			return null;
		}
		var names = [];
	...
	,countRows: function(db,name) {
		if(!db.begin("SELECT COUNT(*) AS ct FROM " + db.getQuotedTableName(name),null,["ct"])) {
			return -1;
		}
		var ct = -1;
	...
	,getRowIDs: function(db,name) {
		var result = [];
		if(!db.begin("SELECT ROWID AS r FROM " + db.getQuotedTableName(name) + " ORDER BY ROWID",null,["r"])) {
			return null;
		}
		while(db.read()) result.push(db.get(0));
		db.end();
		return result;
	}
	...
	,attach: function(db,tag,resource_name) {
		...
		[];
		if(!db.begin("PRAGMA database_list",null,["seq","name","file"])) {
			return false;
		}
		while(db.read()) if(db.get(1) == tag) {
			tag_present = true;
			if(db.get(2) == resource_name) {
				tag_correct = true;
			}
		}
		db.end();
	...
	,fetchSchema: function(db,name) {
		...
		if(!db.begin("select sql from sqlite_master where name = '" + tname + "'",null,["sql"])) {
	...
	,alterColumns: function(db,name,columns) {
		...
		var schema = this.splitSchema(db,name,this.fetchSchema(db,name));
		var nparts = [];
		var new_column_list = [];
		var ins_column_list = [];
		var sel_column_list = [];
...
var coopy_TerminalDiffRender = $hx_exports["coopy"]["TerminalDiffRender"] = function(flags) {
...
coopy_TerminalDiffRender.prototype = {
	...
	,render: function(t) {
		...
		if(__map_reserved.meta != null) {
			_this1.setReserved("meta","\x1B[0;1m");
		} else {
			_this1.h["meta"] = "\x1B[0;1m";
		}
		var _this2 = this.codes;
		if(__map_reserved.spec != null) {
			_this2.setReserved("spec","\x1B[35;1m");
		} else {
			_this2.h["spec"] = "\x1B[35;1m";
		}
		var _this3 = this.codes;
		if(__map_reserved.add != null) {
			_this3.setReserved("add","\x1B[32;1m");
		} else {
			_this3.h["add"] = "\x1B[32;1m";
		}
		var _this4 = this.codes;
		if(__map_reserved.conflict != null) {
			_this4.setReserved("conflict","\x1B[33;1m");
		} else {
			_this4.h["conflict"] = "\x1B[33;1m";
		}
		var _this5 = this.codes;
		if(__map_reserved.modify != null) {
			_this5.setReserved("modify","\x1B[34;1m");
		} else {
			_this5.h["modify"] = "\x1B[34;1m";
		}
		var _this6 = this.codes;
		if(__map_reserved.remove != null) {
			_this6.setReserved("remove","\x1B[31;1m");
		} else {
			_this6.h["remove"] = "\x1B[31;1m";
		}
		var _this7 = this.codes;
		if(__map_reserved.minor != null) {
			_this7.setReserved("minor","\x1B[2m");
		} else {
			_this7.h["minor"] = "\x1B[2m";
		}
		var _this8 = this.codes;
		if(__map_reserved.done != null) {
			_this8.setReserved("done","\x1B[0m");
		} else {
			_this8.h["done"] = "\x1B[0m";
		}
		var sizes = null;
		...
		while(_g1 < h) {
			...
			while(_g3 < w) {
				...
				if(x > 0) {
					...
					var tmp = (__map_reserved.minor != null?_this9.getReserved("minor"):_this9.h["minor"]) + ",";
...
var coopy_Workbook = function() { };
coopy_Workbook.__name__ = true;
coopy_Workbook.prototype = {
	__class__: coopy_Workbook
};
var coopy_Worksheet = function() { };
coopy_Worksheet.__name__ = true;
coopy_Worksheet.prototype = {
	__class__: coopy_Worksheet
};
var coopy_Xlsx = $hx_exports["coopy"]["Xlsx"] = function() {
	this.impl = new coopy_XlsxJs();
};
coopy_Xlsx.__name__ = true;
coopy_Xlsx.prototype = {
	renderTable: function(t) {
		if(this.impl == null) {
			return null;
		}
		var workbook = this.impl.create();
		this.render(workbook,"Sheet",t);
	...
	,renderTables: function(tabs) {
		...
		var order = tabs.getOrder();
		if(order.length == 0 || tabs.hasInsDel()) {
			this.render(workbook,"Sheet",tabs.one());
		}
		var _g1 = 1;
		var _g = order.length;
		while(_g1 < _g) {
			...
			this.render(workbook,name,tab);
		}
		return workbook.getBytes();
	}
	,render: function(workbook,name,tab) {
		...
		var view = tab.getCellView();
		if(coopy_DiffRender.renderCell(tab,view,0,0).category == "") {
			return;
		}
		var _g11 = 0;
		var _g4 = tab.get_width();
		while(_g11 < _g4) {
			var x1 = _g11++;
			var _g31 = 0;
			var _g21 = tab.get_height();
			while(_g31 < _g21) {
				var y1 = _g31++;
				var color = null;
				var cell = coopy_DiffRender.renderCell(tab,view,x1,y1);
				if(cell.category != null) {
					color = this.getColor(cell.category);
				}
				if(color != null) {
					worksheet.setCellFillColor(x1,y1,color);
				}
			}
		}
	...
	,parseTable: function(bytes) {
		if(this.impl == null) {
			return null;
		}
		return coopy_Coopy.tablify(this.impl.read(bytes).getWorksheet(0).getData());
	}
	,__class__: coopy_Xlsx
};
var coopy_XlsxImpl = function() { };
coopy_XlsxImpl.__name__ = true;
coopy_XlsxImpl.prototype = {
	__class__: coopy_XlsxImpl
};
var coopy_XlsxJs = function() {
	if (!require('fibers').current) throw('run inside Fiber plz');
};
coopy_XlsxJs.__name__ = true;
coopy_XlsxJs.__interfaces__ = [coopy_XlsxImpl];
coopy_XlsxJs.prototype = {
	create: function() {
		return new coopy_XlsxJsWorkbook(new exceljs_Workbook());
	}
	,read: function(bytes) {
		var workbook = new exceljs_Workbook();
		var stream = new coopy_ReadableStreamBuffer();
		stream.put(bytes);
		stream.stop();
		require('fibers/future').fromPromise(workbook.xlsx.read(stream)).wait();
		return new coopy_XlsxJsWorkbook(workbook);
	}
	,__class__: coopy_XlsxJs
};
var coopy_XlsxJsWorkbook = function(workbook) {
	this.workbook = workbook;
};
coopy_XlsxJsWorkbook.__name__ = true;
coopy_XlsxJsWorkbook.__interfaces__ = [coopy_Workbook];
coopy_XlsxJsWorkbook.prototype = {
	addWorksheet: function(name) {
		return new coopy_XlsxJsWorksheet(this.workbook.addWorksheet(name));
	}
	,getWorksheet: function(index) {
		return new coopy_XlsxJsWorksheet(this.workbook.getWorksheet(index + 1));
	}
	,getBytes: function() {
		var stream = new coopy_WritableStreamBuffer();
		require('fibers/future').fromPromise(this.workbook.xlsx.write(stream)).wait();
		return stream.getContents();
	}
	,__class__: coopy_XlsxJsWorkbook
};
var coopy_XlsxJsWorksheet = function(worksheet) {
	this.worksheet = worksheet;
};
coopy_XlsxJsWorksheet.__name__ = true;
coopy_XlsxJsWorksheet.__interfaces__ = [coopy_Worksheet];
coopy_XlsxJsWorksheet.prototype = {
	getData: function() {
		var rows = [];
	...
	,setCellValue: function(x,y,value) {
		this.worksheet.getCell(y + 1,x + 1).value = value;
	}
	,setCellFillColor: function(x,y,color) {
		this.worksheet.getCell(y + 1,x + 1).fill = { type : "pattern", pattern : "solid", fgColor : { argb : "FF" + color}};
	}
	,tryFitColumnWidth: function() {
		var _g1 = 0;
		var _g = this.worksheet.columns.length;
		while(_g1 < _g) {
			var i = [_g1++];
			var column = this.worksheet.columns[i[0]];
			var maxLength = [5];
			this.worksheet.eachRow((function(maxLength1,i1) {
				return function(row,rowNumer) {
					maxLength1[0] = Math.max(maxLength1[0],row.getCell(i1[0] + 1).text.length) | 0;
				};
			})(maxLength,i));
			column.width = maxLength[0] * 1.5 | 0;
		}
		return false;
	}
	,setAllRowHeight: function(height) {
		this.worksheet.eachRow(function(row,rowNumber) {
			row.height = height;
		});
	}
	,borderAllCell: function() {
		var border = { style : "thin", color : { argb : "#FF000000"}};
		var borders = { top : border, left : border, bottom : border, right : border};
		this.worksheet.eachRow({ includeEmpty : true},function(row,rowNumber) {
	...
	,__class__: coopy_XlsxJsWorksheet
};
var coopy_ReadableStreamBuffer = require("stream-buffers").ReadableStreamBuffer;
var coopy_WritableStreamBuffer = require("stream-buffers").WritableStreamBuffer;
var exceljs_Workbook = require("exceljs").Workbook;
...
haxe_format_JsonPrinter.prototype = {
	write: function(k,v) {
		...
		case 1:
			...
			break;
		case 2:
			this.buf.b += Std.string(isFinite(v)?v:"null");
			break;
		case 3:
			this.buf.b += Std.string(v);
			break;
		...
		case 6:
			...
			if(c == String) {
				...
				while(tmp.hasNext()) {
					var k1 = tmp.next();
					o[k1] = __map_reserved[k1] != null?v2.getReserved(k1):v2.h[k1];
				}
				this.fieldsString(o,Reflect.fields(o));
			} else if(c == Date) {
				var v3 = v;
				this.quote(HxOverrides.dateStr(v3));
			} else {
				this.fieldsString(v,Reflect.fields(v));
			}
			break;
